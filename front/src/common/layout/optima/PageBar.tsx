...
function CommonPageMenuItems(props: { onClose: () => void }) {
  ...
  const { togglePanel, isPanelOpen } = useLegalDocumentsStore();
  ...
  const handleToggleLegalDocs = (event: React.MouseEvent) => {
    event.stopPropagation();
    togglePanel();
    props.onClose();
  };
  ...
  return <>
    ...
    <MenuItem onClick={handleToggleLegalDocs}>
      <ListItemDecorator><FolderOpenIcon /></ListItemDecorator>
      Legal Documents
      <Typography level="body-xs" sx={{ ml: 'auto', opacity: 0.6 }}>
        {isPanelOpen ? 'ON' : 'OFF'}
      </Typography>
    </MenuItem>
