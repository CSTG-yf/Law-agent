...
class GraphRetriever:
    def __init__(self, graph_store: Neo4jGraphStore = None, llm = None, vector_store: ChromaVectorStore = None):
    ...
    def _get_content_from_vector_store(self, entity_name: str, entity_type: str = None) -> Optional[str]:
        """
        从向量数据库中获取与实体相关的文档内容
                    ...
                    if doc_list:
                        for doc in doc_list:
                            if isinstance(doc, str) and len(doc) > 50:
                                contents.append(doc[:500])
                
                if contents:
                    combined_content = "\n".join(contents[:2])
                    logger.info(f"从向量数据库获取内容 - entity: {entity_name}, content_length: {len(combined_content)}")
                    return combined_content
            
            return None
            
        except Exception as e:
            logger.warning(f"从向量数据库获取内容失败 - entity: {entity_name}, error: {str(e)}")
            return None
    ...
    def _convert_to_documents(
        ...
        for entity in entities:
            ...
            if not content_text and self.vector_store:
                vector_content = self._get_content_from_vector_store(name, entity_type)
                if vector_content:
                    content_text = vector_content
        ...
        for rel in relationships:
            ...
            if not source_content and self.vector_store:
                source_content = self._get_content_from_vector_store(source_name, source.get("type"))
            if source_content:
                content_parts.append(f"源实体内容：{source_content}")
            
            if target.get("description"):
                content_parts.append(f"目标实体描述：{target['description']}")
            
            target_content = target.get("content", "")
            if not target_content and self.vector_store:
                target_content = self._get_content_from_vector_store(target_name, target.get("type"))
            if target_content:
                content_parts.append(f"目标实体内容：{target_content}")
        ...
        for related in related_entities:
            ...
            if not center_content and self.vector_store:
                center_content = self._get_content_from_vector_store(center_name, center.get("type"))
            if center_content:
                content_parts.append(f"中心实体内容：{center_content}")
            
            if neighbor.get("description"):
                content_parts.append(f"关联实体描述：{neighbor['description']}")
            
            neighbor_content = neighbor.get("content", "")
            if not neighbor_content and self.vector_store:
                neighbor_content = self._get_content_from_vector_store(neighbor_name, neighbor.get("type"))
            if neighbor_content:
                content_parts.append(f"关联实体内容：{neighbor_content}")
